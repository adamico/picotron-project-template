local astar = {}
function astar.getPath(original_start_pos, original_end_pos, collisionFunc, usingGrid, scale)
	--map_scale differs between picotron and pico8
	local map_scale = (usingGrid or true) and 1 or (scale or 16)
	...
	if not collisionFunc(end_pos.x, end_pos.y)
		or not collisionFunc(start_pos.x, start_pos.y) then
		return {}
	end
	...
	local function get_neighbors(node)
		...
		local function handle_neighbor(off_x,off_y)
			...
			if not collisionFunc(pos_x,pos_y) or closed_nodes[pos_to_key(pos_x,pos_y)] then
				return
			end
